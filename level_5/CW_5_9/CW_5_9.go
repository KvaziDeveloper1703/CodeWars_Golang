...
func firstNonRepeatingLetter(s string) string {
	...
	for _, char := range s {
		...
		charCount[rune(lowerChar[0])]++
	}

	for _, char := range s {
		lowerChar := strings.ToLower(string(char))
		if charCount[rune(lowerChar[0])] == 1 {
			return string(char)
		}
	}
...
func main() {
	fmt.Println(firstNonRepeatingLetter("stress"))
	fmt.Println(firstNonRepeatingLetter("sTreSS"))
	fmt.Println(firstNonRepeatingLetter("aabbcc"))
	fmt.Println(firstNonRepeatingLetter("αβγδαβ"))
}
